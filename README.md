...
## Games
If you want to test the different optimizers on your own game, use ```QuadGame()``` to generate a quadratic game and ```MatrixGame()``` to generate a matrix game from [model.py](model.py). You need to use the ```grad``` method to return oracles evaluated a particular value for the game.
